...
!(function(factory) {
	...
	ParsleyUI.prototype = {
		...
		setupField: function(fieldInstance) {
			...
			if (!fieldInstance.$element.is(fieldInstance.options.excluded)) { /** Mess with DOM now **/
	...
	ParsleyForm.prototype = {
		...
		_isFieldInGroup: function(field, group) {
			if (ParsleyUtils.isArray(field.options.group)) return -1 !== $.inArray(field.options.group, group);
	...
	$.emit = function(name, instance) {
		...
		for (var i = 0; i < subscribed[name].length; i++) {
			...
			if (subscribed[name][i].instance instanceof ParsleyForm && instance instanceof ParsleyField) for (var j = 0; j < subscribed[name][i].instance.fields.length; j++)
